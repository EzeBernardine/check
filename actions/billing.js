...
export const getClientLedger = async (baseURL, clientLedgerId) => {
  try {
    const { data } = await axios.get(
      baseURL + `/billing/client-ledger/${clientLedgerId}`,
      {
        headers: getHeaders(),
      }
    );
...
export const getPaymentProvider = async (baseURL, clientLedgerId) => {
  try {
    ...
    const { data } = await axios.get(
      baseURL + `/billing/payment-providers/?clientLedgerId=${clientLedgerId}`,
      {
        headers: getHeaders(),
      }
    );
...
export const getPaymentProviderById = async (baseURL, providerId) => {
  try {
    // const {data} = await axios.get(baseURL + `/billing/payment-providers/?clientLedgerId=${clientLedgerId}`,{
    const { data } = await axios.get(
      baseURL + `/billing/payment-providers/${providerId}`,
      {
        headers: getHeaders(),
      }
    );
